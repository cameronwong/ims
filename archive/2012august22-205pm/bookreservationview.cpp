    ...
    QWidget(parent)
{
    ...
    connect(reserveButton, SIGNAL(clicked()), this, SLOT(makeReservation()));
...
{
    // reservation needs customerID, payment method, itemID, time period,
    // price, location, employee ID

    // customer needs first name, last name, phone number, email, increment
    // res count, add to reservations

    // item just needs a refID to the reservation

    QStringList textParameters;
    QList<quint32> numericParameters;
    QDateTime startTime;
    QDateTime endTime;
    QString employeeID = empIdEdit->text();

    textParameters << firstNameEdit->text() << lastNameEdit->text() << emailEdit->text();
    textParameters << phoneEdit->text() << costEdit->text();
    numericParameters << itemEdit->currentIndex() << locationEdit->currentIndex();
    numericParameters << paymentMethodEdit->currentIndex() << employeeID.toULong();
    startTime = startTimeEdit->dateTime();
    endTime = endTimeEdit->dateTime();

    emit sendReservation(textParameters, numericParameters, startTime, endTime);
